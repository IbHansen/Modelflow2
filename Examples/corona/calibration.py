...
def ti_calibrate(mmodel,instrument,target,value,time,delay=0):
    mmodel.keep_solutions = {}
    mmodel.keep_solutions['Before calibration'] = mmodel.lastdf.copy(deep=True)
    targetdf=pd.DataFrame(value,index=[time],columns=instrument)
    ...
    res2 = mmodel(res,silent=0,keep='After calibration')
    return res2
ti_calibrate(mcorona,['PROBABILITY_TRANSMISION'],'INFECTIOUS_GROWTH',3,0,10)
