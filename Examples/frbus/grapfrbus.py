# -*- coding: utf-8 -*-
"""
This i a script which onboards the FRB/US model 
...
if not 'ffrbusvar'   in locals(): 
    ...
    first = [d['neq'] for v,d in frbusdic.items() if d['neq']]  # first extract of definition
    desdic = {v : d['definition'] for v,d in frbusdic.items()}
    typedic = {v : d['type'] for v,d in frbusdic.items()}
    ...
    rescom2a = [f'{a} ' if '_aerr' in a else  f'{a} + {v+"_aerr"} ' for a,v in zip(rescom,vars)]
    rescom2 =  [f'<Z> {a} ' if typedic[v]=='Behavioral' or v.upper().startswith('Z') else f'{a}' for a,v in zip(rescom2a,vars)]
...
#%%    
    ...
    # make the mce model, 
    eqvar = [(e.split('=')[0].strip().split()[-1],e) for e in rescom2]
    lffrbusmce = [f'<Z> {v} = {mcedic[v]}' if v in mcedic.keys() else e for v,e in eqvar]
...
#%%    
if 1: # the mode with forward looking 
    #%% get the mce data     
...
#%%
    ...
    mfrbusmce.use_preorder=1
    #res = mfrbusmce.sim2d(longbase,'2020q1','2115q4',solveorder=mfrbusmce.strongorder,antal= 50,fair=10,silent=1)
    res1 = mfrbusmce.sim2d(longbase,'2020q1','2047q4',dumpvar=['HGEMP','EMP','HGGDPT'],
                          conv='XGDPN',antal = 100,alfa = 0.4,
                  silent=0,ldumpvar=1)
#%%
    fairopt = {'fairconv':'XGDPN','fairantal':200}
    try:
        with mc.ttimer():
            longmcebase = mfrbusmce.sim1d(res1,'2020q1','2070q4',dumpvar=['XGDPN'],
                                  conv='XGDPN',antal = 100,alfa = 1.0,
                                  reverse=False, fairopt=fairopt,silent=1,ldumpvar=1,ljit=1,stats=1)
 #   mfrbusmce.lastdf = res
    except: 
        pass
    
    check = mfrbusmce.dumpdf.sort_values(['per','fair','iteration'])
#        zz = check.loc[:,'per fair iteration FPXR XGDPN FNIN FPI10T FPXRR FRL10'.split()].query('per==@mfrbusmce.periode')
    zz = check.loc[:,'per fair iteration XGDPN'.split()].query('per==@mfrbusmce.periode')
    zz.plot(subplots=True,figsize = (10, 10))
    #%%
...
baseline = frbusdfsol.copy()
with mc.ttimer('Solve the model'):
    res = mfrbus(frbusdfsol,'2020q1','2030q4',sim=True,antal= 500,
                 conv='XGDPN',first_test=10,ldumpvar=True,ljit=False,silent=False,stats=1,debug=1)
itvar = mfrbus.dumpdf
#%%
...
if 1:
    #%% save model and data
    baseline.to_pickle(r'data/baseline.pc') 
    longmcebase.to_pickle(r'data/longmcebase.pc') 
    open('model/ffrbusvar.txt','wt').write(ffrbusvar)
    open('model/ffrbusmce.txt','wt').write(ffrbusmce)
