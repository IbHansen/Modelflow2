...
# ---------------------------------------------------------------------------
# Grid widget (plain ipywidgets, no ipydatagrid)
# ---------------------------------------------------------------------------
...
def _header_cell(text: str, width: str = "100px") -> HTML:
    return HTML(
        ...
        layout=Layout(
            ...
            align_items="center", padding="2px 4px",
...
def _corner_cell(width: str = "100px") -> HTML:
    return HTML(value="", layout=Layout(width=width, min_width=width, border="1px solid #ccc"))
...
def _grid_header_cell(text: str, width: str = "100px", tooltip: str = "") -> HTML:
    import html as _html
    t = _html.escape(str(text))
    tip = _html.escape(str(tooltip))
    return HTML(
        value=(
            f'<div title="{tip}" '
        ...
        layout=Layout(width=width, min_width=width, max_width=width),
    )

def _grid_index_cell(text: str, width: str = "100px") -> HTML:
    import html as _html
    t = _html.escape(str(text))
    return HTML(
        value=(
...
_GRID_ROW_HEIGHT = "32px"

def _grid_header_cell(text: str, width: str = "100px", tooltip: str = "") -> HTML:
    import html as _html
    t, tip = _html.escape(str(text)), _html.escape(str(tooltip))
    return HTML(
        value=(
            f'<div title="{tip}" '
            f'style="font-weight:bold; padding:2px 4px; height:{_GRID_ROW_HEIGHT}; line-height:{_GRID_ROW_HEIGHT}; '
            f'border:1px solid #ddd; box-sizing:border-box; background:#f5f5f5; '
            f'white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">'
            f'{t}</div>'
        ),
        layout=Layout(width=width, min_width=width, max_width=width, height=_GRID_ROW_HEIGHT),
    )
...
def _grid_index_cell(text: str, width: str = "100px") -> HTML:
    ...
    return HTML(
        value=(
            f'<div style="padding:2px 4px; height:{_GRID_ROW_HEIGHT}; line-height:{_GRID_ROW_HEIGHT}; '
            f'border:1px solid #ddd; box-sizing:border-box; background:#fafafa; '
            f'white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">'
            f'{t}</div>'
        ),
        layout=Layout(width=width, min_width=width, max_width=width, height=_GRID_ROW_HEIGHT),
...
class gridwidget(SingleWidgetBase):
    """
    Editable DataFrame grid using only plain ipywidgets (no ipydatagrid).

    Same ``content`` keys as ``sheetwidget``:
    - ``update_col`` + ``update_index``  **or**  ``update_df``
    - ``operator`` (``+``, ``=``, ``*``, ``%``) – default ``+``
    - ``dec`` – decimal places (default 2)

    ``widgetdef`` options:
    - ``transpose`` (bool, default True) – variables become rows
    - ``max_height`` – scroll area height (default "400px")
    - ``max_width``  – scroll area width  (default "100%")
    """
    ...
    _cells: Dict[tuple, Any] = field(init=False, default_factory=dict)
    _datawidget: Any = field(init=False, default=None)
    ...
    def _build_grid(self) -> None:
        ...
        # ---- Left panel: frozen index column ----
        index_cells = [_grid_header_cell("", idx_width)]  # corner
        for r_label in row_labels:
            index_cells.append(_grid_index_cell(r_label, idx_width))

        index_column = VBox(
            index_cells,
            layout=Layout(width=idx_width, min_width=idx_width),
        )

        # ---- Right panel: header + data (scrolls horizontally) ----
        header_cells = [_grid_header_cell(c, cell_width) for c in col_labels]
        header_row = HBox(header_cells, layout=Layout(width="max-content", height=_GRID_ROW_HEIGHT))
        ...
        for r_idx in range(len(row_labels)):
            ...
            for c_idx in range(len(col_labels)):
                ft = FloatText(
                    value=float(df.iloc[r_idx, c_idx]),
                    step=step,
                    layout=Layout(
                        width=cell_width, min_width=cell_width, max_width=cell_width,
                        height=_GRID_ROW_HEIGHT,
                    ),
                    style={"description_width": "0px"},
                )
                self._cells[(r_idx, c_idx)] = ft
                row_widgets.append(ft)
            data_rows.append(HBox(row_widgets, layout=Layout(width="max-content", height=_GRID_ROW_HEIGHT)))
        ...
        max_width = self.widgetdef.get("max_width", "100%")
        ...
        # ---- Combined: index + data side by side, vertical scroll on outer ----
        max_height = self.widgetdef.get("max_height", "400px")

        combined = HBox(
            [index_column, data_scroll],
            layout=Layout(width="max-content"),
        )
        ...
        self._datawidget = VBox([Label(value=self.heading, layout=Layout(width="54%")), outer])
    ...
    def update_df(self, df: pd.DataFrame, current_per: Any = None) -> None:
        ...
        match self.op:
            case "+": df.loc[updated.index, updated.columns] = df_slice + updated
            case "=": df.loc[updated.index, updated.columns] = updated
            case "*": df.loc[updated.index, updated.columns] = df_slice * updated
            case "%": df.loc[updated.index, updated.columns] = df_slice * (1 + updated / 100)
            case _:   raise ValueError(f"Unsupported operator {self.op!r} in gridwidget for {self.heading!r}.")
